...
def load_and_transform_data():
    ...
    def load_and_join_data(**kwargs):
        ...
        con.execute(f"CREATE OR REPLACE TABLE customers AS SELECT * FROM read_csv('{data_dir}/customers.csv', header = true)")
        print(f"Loaded {con.execute('SELECT COUNT(*) FROM customers;').fetchone()[0]} customer records.")

        con.execute(f"CREATE OR REPLACE TABLE transactions AS SELECT * FROM read_json_auto('{data_dir}/transactions/*.json');")

        con.execute("CREATE OR REPLACE TABLE customer_transactions AS SELECT * FROM transactions t JOIN customers c ON t.customer_id = c.customer_id")
